...
rI = 120;
seed = 13;
load(libfile,'dtRange','vRange','sEPSP','sIPSP0','sEPSP0','dur','kVEI','kVIE','kVEE','kVII','fE','fI','nE','l0');
nt0 = size(sEPSP0,1);
nbt = size(kVEI,1);
ndt = size(kVEI,2);
nv = length(vRange);
...
filename = [num2str(rE),'-',num2str(rI),'-Data-test.bin'];
fid = fopen(filename);
if iModel == 0
    outputMat = fread(fid,[run_nt,8],'double');
%     outputMat = fread(fid,[run_nt,9],'double');
else
    outputMat = fread(fid,[run_nt,5],'double');
%     outputMat = fread(fid,[run_nt,6],'double');
end
fclose(fid);
...
gE   = outputMat(:,4);
gI   = outputMat(:,5);
filename = [num2str(rE),'-',num2str(rI),'-tIn-test.bin'];
fid = fopen(filename);
ntmp = fread(fid,[1,1],'uint');
tin = fread(fid,[ntmp,1],'double');
tID = fread(fid,[ntmp,1],'int64');
fclose(fid);
tID = tID + 1;
pE = (tID <= nE);
pI = (tID > nE);
