function [sEPSP,sIPSP,t] = single_check(name,pick,model,picformat,draw,ppp,loadData)
    global tstep printDriver dpi
    visible = true;
    if ~visible
        set(0,'DefaultFigureVisible','off');
    end
    cd('S:\Nutstore\Projects\PhD\PNM\PNM_dev');
...
%      ppp = true;
    v0 = -0.4:0.1:1.4;
%     v0 = [-0.4, -0.3,-0.15, 0, 0.4, 0.8, 1.2, 1.4, 1.6];
    v0id = find(v0 == 0);
    assert(~isempty(v0id));
    nv0 = length(v0);
...
%     fI = 0.5*1e-6;
    ...
    for j = 1:l    
        ...
        if draw
            ...
            printpic(h,fname,picformat,printDriver,dpi,pos0);
        end
        %
        if draw
            h = figure;
...
%%      wei's approach, PSP = volatage trace - leakage trace at v0 = v;
        ...
        v0case = [1,round(nv0/2),nv0,find(v0==0)];
        ...
        for iv0 = 1:nv0
            ...
            for idt = 1:ndt
                ...
                if sum((iv0 - v0case) == 0) == 1 && draw && ppp
                    pp0 = true;
                else
                    pp0 = false;
                end
                if testEE
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kEE(:,idt,iv0),pv,vadd,vEEDoublet,vEE,kEE0(:,idt),pv0,vadd0,vEE0,iQ,kQ,pQ,vE,vE2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sEPSP(:,:,iv0),sEPSP,vRange,iv0,pp0);
                    else                                                                 
                        rest = false;                                                   
                        [kEE(:,idt,iv0),pv,vadd,vEEDoublet,vEE,          ~,  ~,    ~,vEE0,iQ,kQ,pQ,vE,vE2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sEPSP(:,:,iv0),sEPSP,vRange,iv0,pp0);
                    end
                    kQEE(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nE]);
                    if sum((iv0 - v0case) == 0) == 1 && draw
                        hEE0(idt) = drawExample(hEE0(idt),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vEEDoublet,vleakage(:,iv0),t,iidt,tp0,kEE(:,idt,iv0),kEE0(:,idt),kQ,nE*nE,vRange,para.vRest(i),vEE,vEE0,'V_{E}V_{E}','EE');
                    end
                    if sum((iv0 - v0case) == 0) == 1 && draw && ppp
                ...
                if testII
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kII(:,idt,iv0),pv,vadd,vIIDoublet,vII,kII0(:,idt),pv0,vadd0,vII0,iQ,kQ,pQ,vI,vI2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sIPSP(:,:,iv0),sIPSP,vRange,iv0,pp0);
                    else                                                                 
                        rest = false;                                                   
                        [kII(:,idt,iv0),pv,vadd,vIIDoublet,vII,          ~,  ~,    ~,vII0,iQ,kQ,pQ,vI,vI2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sIPSP(:,:,iv0),sIPSP,vRange,iv0,pp0);
                    end
                    kQII(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nI]);
                    if sum((iv0 - v0case) == 0) == 1 && draw
                        hII0(idt) = drawExample(hII0(idt),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vIIDoublet,vleakage(:,iv0),t,iidt,tp0,kII(:,idt,iv0),kII0(:,idt),kQ,nI*nI,vRange,para.vRest(i),vII,vII0,'V_{I}V_{I}','II');
                    end
                    if sum((iv0 - v0case) == 0) == 1 && draw && ppp
                ...
                if testEI
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kEI(:,idt,iv0),pv,vadd,vEIDoublet,vEI,kEI0(:,idt),pv0,vadd0,vEI0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sEPSP(:,:,iv0),sIPSP,vRange,iv0,pp0);
                    else                                                                 
                        rest = false;                                                   
                        [kEI(:,idt,iv0),pv,vadd,vEIDoublet,vEI,          ~,  ~,    ~,vEI0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sEPSP(:,:,iv0),sIPSP,vRange,iv0,pp0);
                    end
                    kQEI(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nE]);
                    if sum((iv0 - v0case) == 0) == 1 && draw
                        hEI0(idt) = drawExample(hEI0(idt),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vEIDoublet,vleakage(:,iv0),t,iidt,tp0,kEI(:,idt,iv0),kEI0(:,idt),kQ,nE*nI,vRange,para.vRest(i),vEI,vEI0,'V_{E}V_{I}','EI');
                    end
                    if sum((iv0 - v0case) == 0) == 1 && draw && ppp
                ...
                if testIE
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kIE(:,idt,iv0),pv,vadd,vIEDoublet,vIE,kIE0(:,idt),pv0,vadd0,vIE0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sIPSP(:,:,iv0),sEPSP,vRange,iv0,pp0);
                    else                                                                 
                        rest = false;                                                   
                        [kIE(:,idt,iv0),pv,vadd,vIEDoublet,vIE,          ~,  ~,    ~,vIE0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage(:,iv0),sIPSP(:,:,iv0),sEPSP,vRange,iv0,pp0);
                    end
                    kQIE(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nI]);
                    if sum((iv0 - v0case) == 0) == 1 && draw
                        hIE0(idt) = drawExample(hIE0(idt),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vIEDoublet,vleakage(:,iv0),t,iidt,tp0,kIE(:,idt,iv0),kIE0(:,idt),kQ,nE*nI,vRange,para.vRest(i),vIE,vIE0,'V_{I}V_{E}','IE');
                    end
                    if sum((iv0 - v0case) == 0) == 1 && draw && ppp
        ...
        if draw
              %  paper
                ...
                if kQplot
                    if testEE
                        ...
                        printpic(hQEE,fname,picformat,printDriver,dpi,pos);
                    end
                    if testII
                        fname = ['kII','-',name,'-',num2str(i),'-',model];
                        printpic(hQII,fname,picformat,printDriver,dpi,pos);
                    end
                    if testEI
                        fname = ['kEI','-',name,'-',num2str(i),'-',model];
                        printpic(hQEI,fname,picformat,printDriver,dpi,pos);
                    end
                    if testIE
                        fname = ['kIE','-',name,'-',num2str(i),'-',model];
                        printpic(hQIE,fname,picformat,printDriver,dpi,pos);
                    end
                end
            ...
            for idt = 1:ndt
                if testEE
                    ...
                    printpic(hEE0(idt),fname,picformat,printDriver,dpi,pos0);
                end
                if testII
                    fname = ['II',num2str(idt),'_example','-',name,'-',num2str(i),'-',model];
                    printpic(hII0(idt),fname,picformat,printDriver,dpi,pos0);
                end
                if testEI
                    fname = ['EI',num2str(idt),'_example','-',name,'-',num2str(i),'-',model];
                    printpic(hEI0(idt),fname,picformat,printDriver,dpi,pos0);
                end
                if testIE
                    fname = ['IE',num2str(idt),'_example','-',name,'-',num2str(i),'-',model];
                    printpic(hIE0(idt),fname,picformat,printDriver,dpi,pos0);
                end
            end
            %if meshK
            ...
            %    printpic(hEI,fname,picformat,printDriver,dpi,pos0);
            %end
        end
...
%         durpsp = dur-dtstep;
        ...
        test = true;
        fname = ['test',num2str(dur),'-E',num2str(rateE*1e3),'-I',num2str(rateI*1e3),'-S',num2str(seed),'.mat'];
        if test && exist(fname,'file')
            load(fname,'tmp');
        else
            tic;
...
%         rule = @(iiE,iiI,iE,iI) iiI == 10 && iE == 26;
        ...
        for iE = 1:nfE
            while para.tI(iI) < para.tE(iE) && ~finishI
                ...
                for iiE = 1: iE-1
                    if para.tI(iI) - para.tE(iiE) < durpsp
                        ...
                        [vpred0, vpred1, vpred2, bEI0{iiE,iI},bEI1{iiE,iI},bEI2{iiE,iI},tEI(:,iiE,iI)] = pred(vpred0,vpred1,vpred2,vadd,vadd,iiE,iI,vE0,vE1,vE2,vI0,vI1,vI2,tEs,tEe,tEl,tIs,kEI0,kEI,kQEI,pfE,vRange,nv0,ndt,ldt);
                        if debug && rule(iiE,0,0,iI)
                ...
                for iiI = 1: iI-1
                    if para.tI(iI) - para.tI(iiI) < durpsp
                        ...
                        [vpred0, vpred1, vpred2, bII0{iiI,iI},bII1{iiI,iI},bII2{iiI,iI},tII(:,iiI,iI)] = pred(vpred0,vpred1,vpred2,vadd,vadd,iiI,iI,vI0,vI1,vI2,vI0,vI1,vI2,tIs,tIe,tIl,tIs,kII0,kII,kQII,pfI,vRange,nv0,ndt,ldt);
                        if debug && rule(0,iiI,0,iI)
            ...
            for iiI = 1:iI-1
                if para.tE(iE) - para.tI(iiI) < durpsp
                    ...
                    [vpred0, vpred1, vpred2, bIE0{iiI,iE},bIE1{iiI,iE},bIE2{iiI,iE},tIE(:,iiI,iE)] = pred(vpred0,vpred1,vpred2,vadd,vadd,iiI,iE,vI0,vI1,vI2,vE0,vE1,vE2,tIs,tIe,tIl,tEs,kIE0,kIE,kQIE,pfI,vRange,nv0,ndt,ldt);
                    if debug && rule(0,iiI,iE,0)
            ...
            for iiE = 1:iE-1
                if para.tE(iE) - para.tE(iiE) < durpsp
                    ...
                    [vpred0, vpred1, vpred2, bEE0{iiE,iE},bEE1{iiE,iE},bEE2{iiE,iE},tEE(:,iiE,iE)] = pred(vpred0,vpred1,vpred2,vadd,vadd,iiE,iE,vE0,vE1,vE2,vE0,vE1,vE2,tEs,tEe,tEl,tEs,kEE0,kEE,kQEE,pfE,vRange,nv0,ndt,ldt);
                    if debug && rule(iiE,0,iE,0)
        ...
        errorbar(1.9,sign(sum(vpred0-v>0)*2-1)*mean(abs(vpred0-v)),std(abs(vpred0-v)),'*b');
        errorbar(2.0,sign(sum(vpred1-v>0)*2-1)*mean(abs(vpred1-v)),std(abs(vpred1-v)),'*m');
        errorbar(2.1,sign(sum(vpred2-v>0)*2-1)*mean(abs(vpred2-v)),std(abs(vpred2-v)),'*g');
        set(ax,'XTick',[1,2]);
        set(ax,'XTickLabel',{'linear';'bilinear'});
        ylabel('error mV');
        xlim([0.5,2.5]);
        subplot(2,2,4)
        hold on
...
function [k,pv,vadd,vDoublet,EI,k0,pv0,vadd0,EI0,iQ,kQ,pQ,v1,v2,h] = doubleCheck(silico,name,para,v0,bool,tstep,iidt,dur,i,rest,vleakage,sPSP1,sPSP2,vRange,iv0,pp)
     if rest > 0
...
%      ndt = round(dur/dtstep)+1;
     ...
     vDoublet = vDoublet - repmat(vleakage,[1,n1*n2]);
     ...
     for iF = 1:n1
         ...
         if rest
            v20(range,:) = [zeros(iidt-1,n2);sPSP2(1:nt-iidt+1,:,iv0)]';
            vadd0(range,:) = v1(range,:) + v20(range,:);
         end
         for it = 1:nt
     ...
     for it = 1:nt
         ...
         if rest
            [k0(it),pv0(:,it)] = p_fit110k0(v1(:,it),v20(:,it),vDoublet(it,:)');
         end
     end
     EI = v1.*v2;
     EI0 = v1.*v20;
...
function h = drawExample(h,pv0,vadd0,pvR0,vaddR0,pQ,iv0,v0case,vDoublet0,vleakage,t,iidt,tp0,k,kR,kQ,n,vRange,vRest,vEI,vEIR,xl,titl)
    nCase = length(v0case);
    ...
    if jv0 < nCase
        ...
        hold on
        p = plot(vEI(:,tp),(vDoublet0(tp,:)'-vadd0(:,tp)),'o');
        xx = sort(vEI(:,tp));
        ...
        p = plot(vEIR(:,tp),(vDoublet0(tp,:)'-vaddR(:,tp)),'sm');
        xx = sort(vEIR(:,tp));
...
function printpic(h,fname,picformat,printDriver,dpi,pos)
    if ~isempty(picformat)
    ...
    % %     set(h,'PaperPositionMode','auto');
        if ~strcmp(picformat,'fig')
            print(h,[fname,'.',picformat],printDriver,'-loose',dpi);
        end
        saveas(h,[fname,'.fig']);
        close(h);
    end
end
function [vpred0, vpred1, vpred2, b0, b1, b2, t] = pred(vpred0,vpred1,vpred2,v1,v2,i1,i2,v10,v11,v12,v20,v21,v22,t1s,t1e,t1l,t2s,k0,k,kQ,pf,vRange,nv0,ndt,ldt)
    lt = t1e(i1)-t2s(i2);
    assert(lt>0);
    tpick = t2s(i2)+(0:lt);
    t(1) = tpick(1);
