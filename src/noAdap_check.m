function [sEPSP,sIPSP,t] = noAdap_check(name,pick,model,picformat,draw,ppp,loadData)
...
%     fI = 0.5*1e-6;
    ...
    dur = 220;
    dtstep = dur/11;
    dtRange = (0:dtstep:(dur-dtstep));
    ndtplot = 3;
    tp0 = round(22/tstep);
    t = 0:tstep:dur;
    nt = round(dur/tstep)+1;
    ndt = round(dur/dtstep);
    assert(ndt==length(dtRange));
    % 1 2 3
    % E I EI
    i = pick;
    diri = [dir,'/',num2str(i)];
    ...
    if ~loadData
        ...
        [h,sEPSP,sIPSP,vleakage] = sPSP_check(silico,vRange,fI',fE',para,bool,name,dur,i,v0id,dtRange);
            save(['single-',name,'.mat'],'h','sEPSP','sIPSP','vleakage');
        else
            load(['single-',name,'.mat']);
        end
        if ~strcmp(model,'HH')
            inv0 = nv0;
            para.vtime = -1;
        ...
        copyfile('noAdap_check.m',[dir,'/noAdap_check-',name,'-',num2str(i),'th.m']);
        %
        if draw
            h = figure;
...
%             vadd0 = zeros(nE*nI,nt);
            ...
            for idt = 1:ndt
                ...
                if (idt==1 || sum((idt-[1,2,7])==0)==1) && draw
                    dtplot = true;
                    idtplot = idtplot+1;
                else
                    dtplot = false;
                ...
                if testEE
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kEE(:,idt,iv0),pv,vadd,vEEDoublet,vEE,kEE0(:,idt),pv0,vadd0,vEE0,iQ,kQ,pQ,vE,vE2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sEPSP,sEPSP,vRange,iv0,pp0,v0id,idt,dtRange);
                    else                                                                 
                        rest = false;                                                   
                        [kEE(:,idt,iv0),pv,vadd,vEEDoublet,vEE,kbEE0(:,:,idt),pv0,vadd0,vEE0,iQ,kQ,pQ,vE,vE2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sEPSP,sEPSP,vRange,iv0,pp0,v0id,idt,dtRange,kEE0(:,idt));
                    end
                    kQEE(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nE]);
                    if v0plot && dtplot
                        hEE0(idtplot) = drawExample(hEE0(idtplot),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vEEDoublet,vleakage(:,1,1,iv0),t,iidt,tp0,kEE(:,idt,iv0),kbEE0(:,:,idt),kEE0(:,idt),kQ,nE*nE,vRange,para.vRest(i),vEE,vEE0,'V_{E}V_{E}','EE',draw0);
                    end
                    if pp0
                ...
                if testII
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kII(:,idt,iv0),pv,vadd,vIIDoublet,vII,kII0(:,idt),pv0,vadd0,vII0,iQ,kQ,pQ,vI,vI2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sIPSP,sIPSP,vRange,iv0,pp0,v0id,idt,dtRange);
                    else                                                                 
                        rest = false;                                                   
                        [kII(:,idt,iv0),pv,vadd,vIIDoublet,vII,kbII0(:,:,idt),pv0,vadd0,vII0,iQ,kQ,pQ,vI,vI2,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sIPSP,sIPSP,vRange,iv0,pp0,v0id,idt,dtRange,kII0(:,idt));
                    end
                    kQII(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nI]);
                    if v0plot && dtplot
                        hII0(idtplot) = drawExample(hII0(idtplot),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vIIDoublet,vleakage(:,1,1,iv0),t,iidt,tp0,kII(:,idt,iv0),kbII0(:,:,idt),kII0(:,idt),kQ,nI*nI,vRange,para.vRest(i),vII,vII0,'V_{I}V_{I}','II',draw0);
                    end
                    if pp0
                ...
                if testEI
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kEI(:,idt,iv0),pv,vadd,vEIDoublet,vEI,kEI0(:,idt),pv0,vadd0,vEI0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sEPSP,sIPSP,vRange,iv0,pp0,v0id,idt,dtRange);
                    else                                                                 
                        rest = false;                                                   
                        [kEI(:,idt,iv0),pv,vadd,vEIDoublet,vEI,kbEI0(:,:,idt),pv0,vadd0,vEI0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sEPSP,sIPSP,vRange,iv0,pp0,v0id,idt,dtRange,kEI0(:,idt));
                    end
                    kQEI(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nE]);
                    if v0plot && dtplot
                        hEI0(idtplot) = drawExample(hEI0(idtplot),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vEIDoublet,vleakage(:,1,1,iv0),t,iidt,tp0,kEI(:,idt,iv0),kbEI0(:,:,idt),kEI0(:,idt),kQ,nE*nI,vRange,para.vRest(i),vEI,vEI0,'V_{E}V_{I}','EI',draw0);
                    end
                    if pp0
                ...
                if testIE
                    ...
                    if iv0 == v0id
                        rest = true;
                        [kIE(:,idt,iv0),pv,vadd,vIEDoublet,vIE,kIE0(:,idt),pv0,vadd0,vIE0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sIPSP,sEPSP,vRange,iv0,pp0,v0id,idt,dtRange);
                    else                                                                 
                        rest = false;                                                   
                        [kIE(:,idt,iv0),pv,vadd,vIEDoublet,vIE,kbIE0(:,:,idt),pv0,vadd0,vIE0,iQ,kQ,pQ,vE,vI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,rest,vleakage,sIPSP,sEPSP,vRange,iv0,pp0,v0id,idt,dtRange,kIE0(:,idt));
                    end
                    kQIE(:,idt,iv0,:) = reshape(kQ',[nt,1,1,nI]);
                    if v0plot && dtplot
                        hIE0(idtplot) = drawExample(hIE0(idtplot),pv,vadd,pv0,vadd0,pQ,iv0,v0case,vIEDoublet,vleakage(:,1,1,iv0),t,iidt,tp0,kIE(:,idt,iv0),kbIE0(:,:,idt),kIE0(:,idt),kQ,nE*nI,vRange,para.vRest(i),vIE,vIE0,'V_{I}V_{E}','IE',draw0);
                    end
                    if pp0
...
function [k,pv,vadd,vDoublet,EI,kb0,pv0,vadd0,EI0,iQ,kQ,pQ,v1,v2,h] = doubleCheck(silico,name,para,v0,bool,tstep,iidt,dur,i,rest,vleakage,sPSP1,sPSP2,vRange,iv0,pp,v0id,idt,dtRange,k0)
     n1 = size(sPSP1,2);
     n2 = size(sPSP2,2);
     nt = round(dur/tstep)+1;
     ndt = length(dtRange);
     ...
     vDoublet = vDoublet - repmat(vleakage(:,1,1,iv0),[1,n1*n2]);
     vadd = zeros(n1*n2,nt);
     v1 = zeros(n1*n2,nt);
     v2 = zeros(n1*n2,nt);
     pQ = zeros(n1*n2,nt);
     ...
     for iF = 1:n1
         range = (iF-1)*n2+(1:n2);
         v1(range,:) = (sPSP1(:,iF,1,1,iv0)*ones(1,n2))';
         [interpPSP2, vt1,vt2, ~] = interpPSP(squeeze(sPSP2(:,:,1,1,:)),vTarget(range(1)),vRange);
         ...
         if rest
            ...
            v10(range,:) = (sPSP1(:,iF,1,1,v0id)*ones(1,n2))';
         end
         v20(range,:) = [zeros(iidt-1,n2);sPSP2(1:nt-iidt+1,:,1,1,v0id)]';
     ...
     if pp
         ...
         for iiv0 = 1:nv0
            ...
            for iF = 1:n1
                ...
                parfor it = 1:nt
                   [kQV(iF,it,iiv0), pQV(range,it,iiv0)] = p_fit110k0(v1V(range,it,iiv0),v2V(range,it,iiv0),vDoubletV(it,range,iiv0)');
                end
                pQV0(range,:,iiv0) = vaddV(range,:,iiv0) + kQV(iF,iQV(iF,iiv0),iiv0)*v1V(range,:,iiv0).*v2V(range,:,iiv0);
            end
            parfor it = 1:nt
