...
save(['parameters-',name,'.mat'],'para','bool','type','species','posInBrain','n');
n
%%
% name = 'LTS_associa_Cat';
...
% tstep = 0.05;
% dur = 1500;
% % y=RK4(name,v0,para,bool,n,tstep,dur);
% % vRest = y(1,end,1);
% % save(['parameters-',name,'.mat'],'para','bool','type','species','posInBrain','n');
% noAdap = true;
noAdap =false;
...
% % for vT = -60
% %     para.vT = vT;
% %     v0Percent = (-60-para.vLeak)/(para.vT-para.vLeak);
%     for fCurrent = [1e-4]
%         fCurrent
        trace_plot(0.05,f_E,f_I,v0Percent,1000,fCurrent,name,'',draw,vonly,returnVonly,@RK4,cplot,noAdap);
%     end
% end
% end
% para.DeltaT = 0.4375*ones(n,1);
...
figure;
hold on
vIF = RK2_IF_df(name,v0,para,bool,tstep,dur,ipick,false);
vEIF = RK2_EIF_df(name,v0,para,bool,tstep,dur,ipick,false);
plot(t,[vIF',vEIF']);
legend({'IF','EIF'});
