function y = RK4_df_vjump(name,v0,it,vt,para,bool,tstep,etime,ipick,savedata)  %RK4 method
if nargin < 8
    savedata = false;
end
stime=0;
...
for i=1:count-1
    ...
    if it == i && i~=1
        v = ones(nf,1)*vt;
        yold(:,1) = v;
        m=m_inf(v,para.vT(ipick));
        n=n_inf(v,para.vT(ipick));
        h=h_inf(v,para.vT(ipick));
        p=p_inf(v);
        q=q_inf(v);
        r=r_inf(v);
