function [sEPSP,sIPSP,t] = noAdapV(name,pick,model,picformat,draw,ppp,loadData)
...
%    test = false;
    test = true;
    tstep = 0.1;
    l0 = 180; % estimating length
    dur = 220; % bilinear k length
    dur0 = 300; % linear length
    dtRange = [0:2:12,15:5:30,50,70,110,150,190];
    ndt = length(dtRange);
...
%     fI = 0.5*1e-6;
    ...
    if ~loadData
        ...
        if ~singleStored
        tic;
        [sEPSP,sIPSP,vleakage] = sPSP_check(silico,tstep,vRange,fI',fE',para,bool,name,dur,i,v0id,dtRange);
        [h,sEPSP0,sIPSP0,~] = sPSP0_check(silico,tstep,vRange,fI',fE',para,bool,name,dur0,i,v0id);
            save(['single-',name,'-',num2str(i),'-v.mat'],'h','sEPSP','sIPSP','sEPSP0','sIPSP0','vleakage');
        else
            load(['single-',name,'-',num2str(i),'-v.mat']);
        end
        toc;
        if ~strcmp(model,'HH')
            inv0 = nv0;
...
%         tQI = round(mean(squeeze(tQI)));
        ...
        kVEE  = zeros(nt,ndt,nv0);
        kVII  = zeros(nt,ndt,nv0);
        kVEI  = zeros(nt,ndt,nv0);
        kVIE  = zeros(nt,ndt,nv0);
...
%         vadd0 = zeros(nE*nI,nt);
        ...
        for idt = 1:ndt
            ...
            if testEE
                ...
                [kVEE(:,idt,:),pv,vadd,vEEDoublet,vEE,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sEPSP,sEPSP0,vRange,pp0,idt);
                if dtplot
                    hEE0(idtplot) = drawExample(hEE0(idtplot),pv,vadd,vEEDoublet,squeeze(vleakage(:,idt,:)),t,iidt,tp0,kVEE(:,idt,:),nE*nE,vRange,para.vRest(i),vEE,'V_{E}V_{E}','EE');
                end
                if pp0
            ...
            if testII
                ...
                [kVII(:,idt,:),pv,vadd,vIIDoublet,vII,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sIPSP,sIPSP0,vRange,pp0,idt);
                if dtplot
                    hII0(idtplot) = drawExample(hII0(idtplot),pv,vadd,vIIDoublet,squeeze(vleakage(:,idt,:)),t,iidt,tp0,kVII(:,idt,:),nI*nI,vRange,para.vRest(i),vII,'V_{I}V_{I}','II');
                end
                if pp0
            ...
            if testEI
                ...
                [kVEI(:,idt,:),pv,vadd,vEIDoublet,vEI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sEPSP,sIPSP0,vRange,pp0,idt);
                if dtplot
                    hEI0(idtplot) = drawExample(hEI0(idtplot),pv,vadd,vEIDoublet,squeeze(vleakage(:,idt,:)),t,iidt,tp0,kVEI(:,idt,:),nE*nI,vRange,para.vRest(i),vEI,'V_{E}V_{I}','EI');
                end
                if pp0
            ...
            if testIE
                ...
                [kVIE(:,idt,:),pv,vadd,vIEDoublet,vIE,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sIPSP,sEPSP0,vRange,pp0,idt);
                if dtplot
                    hIE0(idtplot) = drawExample(hIE0(idtplot),pv,vadd,vIEDoublet,squeeze(vleakage(:,idt,:)),t,iidt,tp0,kVIE(:,idt,:),nE*nI,vRange,para.vRest(i),vIE,'V_{I}V_{E}','IE');
                end
                if pp0
        ...
        if draw
            ...
            if meshK
                ...
                if testIE
                    ...
                    hold on
                    for iv0 = 1:nv0
                        mesh(X,Y,kVIE(:,:,iv0),'EdgeColor',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('I(t1)-E(t2) (ms)');
                    zlabel('k');
                    zlim(klim);
                    fname = ['kVIEmesh','-',name,'-',num2str(i)];
                    printpic(hIE,diri,fname,picformat,printDriver,dpi,pos0);
                end
                if testEI
                    hEI = figure;
                    hold on
                    for iv0 = 1:nv0
                        mesh(X,Y,kVEI(:,:,iv0),'EdgeColor',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('E(t1)-I(t2) (ms)');
                    zlabel('k');
                    zlim(klim);
                    fname = ['kVEImesh','-',name,'-',num2str(i)];
                ...
                if testEE
                    ...
                    hold on
                    for iv0 = 1:nv0
                        mesh(X,Y,kVEE(:,:,iv0),'EdgeColor',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('E(t1)-E(t2) (ms)');
                    zlabel('k');
                    zlim(klim);
                    fname = ['kVEEmesh','-',name,'-',num2str(i)];
                    printpic(hEE,diri,fname,picformat,printDriver,dpi,pos0);
                end
                if testII
                    hII = figure;
                    hold on
                    for iv0 = 1:nv0
                        mesh(X,Y,kVII(:,:,iv0),'EdgeColor',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('I(t1)-I(t2) (ms)');
                    zlabel('k');
                    zlim(klim);
                    fname = ['kVIImesh','-',name,'-',num2str(i)];
                    printpic(hII,diri,fname,picformat,printDriver,dpi,pos0);
                end
            end
        end
        disp('kV generated');
        save([name,'-',num2str(i),'th-v'],'kVEE','kVII','kVEI','kVIE','tp0','vleakage','sEPSP','sIPSP','dur','vRange','fE','fI','nE','nI','i','dtRange','sEPSP0','sIPSP0');
    else
        if multipleInput
            load([name,'-',num2str(i),'th-v']);
            disp('kV loaded');
        end
    end
    if multipleInput
...
%         seed = 122435;
        ...
        if ~simpleTest
            ...
            para.tI = 25;
            dur = dur0 + para.tI;
            fname = ['simpleTest',num2str(dur),'-E',num2str(para.tE),'-I',num2str(para.tI),'-',name,'-',num2str(i),'th'];
        end
                ...
                if testEI
                    for iiE = 1: iE-1
                        if para.tI(iI) - para.tE(iiE) < durpsp
                            ...
                            [vpred, bEI{iiE,iI},tEI(:,iiE,iI)] = pred(vpred,iiE,iI,vE,vI,...
                ...
                if testII
                    for iiI = 1: iI-1
                        if para.tI(iI) - para.tI(iiI) < durpsp
                            ...
                            [vpred, bII{iiI,iI},tII(:,iiI,iI)] = pred(vpred,iiI,iI,vI,vI,...
            ...
            if testIE
                for iiI = 1:iI-1
                    if para.tE(iE) - para.tI(iiI) < durpsp
                        ...
                        [vpred, bIE{iiI,iE},tIE(:,iiI,iE)] = pred(vpred,iiI,iE,vI,vE,...
            ...
            if testEE
                for iiE = 1:iE-1
                    if para.tE(iE) - para.tE(iiE) < durpsp
                        ...
                        [vpred, bEE{iiE,iE},tEE(:,iiE,iE)] = pred(vpred,iiE,iE,vE,vE,...
...
function [EPSP,IPSP,vleakage] = sPSP_check(silico,tstep,vRange,fIRange,fERange,para,bool,name,dur,i,v0id,dtRange)
    ndt = length(dtRange);
    nv0 = length(vRange);
    nE = length(fERange);
    nI = length(fIRange);
    ...
    for iv0 = 1:nv0
        ...
        parfor idt = 1:ndt
            ...
            EPSP(:,:,idt,iv0) = squeeze(tmpE(1,:,:)) - repmat(vleakage(:,idt,iv0),[1,nE]);
        end
        ...
        parfor idt = 1:ndt
            ...
            IPSP(:,:,idt,iv0) = squeeze(tmpI(1,:,:)) - repmat(vleakage(:,idt,iv0),[1,nI]);
        end
    end
    disp('sPSP complete');
end
...
function [kV,pV,vaddV,vDoubletV,EI,h] = doubleCheck(silico,name,para,v0,bool,tstep,iidt,dur,i,vleakage,sPSP1,sPSP2,vRange,pp,idt)
     n1 = size(sPSP1,2);
     n2 = size(sPSP2,2);
     nt = round(dur/tstep)+1;
     nv0 = length(vRange);
     ...
     for iv0 = 1:nv0
        for iF = 1:n1
            range = (iF-1)*n2+(1:n2);
            v1V(range,:,iv0) = (sPSP1(:,iF,idt,iv0)*ones(1,n2))';
            v2V(range,:,iv0) = [zeros(iidt-1,n2);sPSP2(1:nt-iidt+1,:,iv0)]';
        end
        vaddV(:,:,iv0) = v1V(:,:,iv0) + v2V(:,:,iv0);
        parfor it = 1:nt
            [kV(it,iv0),pV(:,it,iv0)] = p_fit110k0(v1V(:,it,iv0),v2V(:,it,iv0),vDoubletV(it,:,iv0)');
        end
     end
     ...
     if pp 
        ...
        t = 0:tstep:dur;
        subplot(3,n2,1)
        text(-0.2,1.2,['linear sum in dot'],'Units','Normalized');
        subplot(3,n2,1*n2+1)
        ...
        for i2 =1:n2
            ...
            hold on
            plot(t,v1V(range(1),:,iv0),':r');
            plot(t,v2V(range(i2),:,iv0),':b');
            plot(t,vaddV(range(i2),:,iv0),':k');
            plot(t,vDoubletV(:,range(i2),iv0),'k');
            subplot(2,n2,n2+i2);
            hold on
            pick = iidt:nt;
            ...
            plot(t(pick),pV(range(i2),pick,iv0),'--k');
            [ax,h1,h2] = plotyy(t(pick),vDoubletV(pick,range(i2),iv0),t(pick),kV(pick,iv0));
            h1.Color = 'k';
            h2.LineStyle = ':';
            h2.Color = 'g';
            hold(ax(2),'on');
...
function [vpred, b, t] = pred(vpred,i1,i2,v1,v2,...
                              t1s,t1e,t1l,t2s,k,...
                              vRange,nv0,ndt,idtRange)
    lt = t1e(i1)-t2s(i2);
    assert(lt>0);
    ...
    kVtmp = interpKV(k,vpred(t2s(i2)),t2s(i2)-t1s(i1)+1,vRange,nv0,idtRange,ndt,lt);
    b(:,1) = kVtmp.*v1((t1l(i1)-lt):t1l(i1),i1).*v2(1:lt+1,i2);
    b(:,2) = kVtmp;
    vpred(tpick) = vpred(tpick) + b(:,1);
    t(1) = tpick(1); t(2) = tpick(end);
end

function kVtmp = interpKV(k,vTarget,tTarget,v,nv0,idtRange,ndt,lt)
