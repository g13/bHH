function [sEPSP,sIPSP,t] = noAdapV(theme,name,pick,model,picformat,draw,ppp,loadData,npool,v0,fE,fI,avoidSpike)
...
%    simpleTest = true;
    ...
    test = false;
%    test = true;
    ddtRange = diff([dtRange,dur]);
    for j=2:ndt
        assert(ddtRange(j)>=ddtRange(j-1));
    end
    ...
    if ~loadData
        if ~singleStored
            tic;
            [sEPSP,sIPSP,E_tmax,I_tmax,vleakage] = sPSP_check(silico,tstep,vRange,fI',fE',para,bool,name,dur,i,v0id,dtRange,avoidSpike);
            toc;
            tic;
            if dur==dur0
...
%         vadd0 = zeros(nE*nI,nt);
        ...
        for idt = 1:ndt
            ...
            if testEE
                ...
                [kVEE(:,idt,:),pv,vadd,vEEDoublet,vEE,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sEPSP,sEPSP0,vRange,pp0,idt,avoidSpike);
                if dtplot
            ...
            if testII
                ...
                [kVII(:,idt,:),pv,vadd,vIIDoublet,vII,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sIPSP,sIPSP0,vRange,pp0,idt,avoidSpike);
                if dtplot
            ...
            if testEI
                ...
                [kVEI(:,idt,:),pv,vadd,vEIDoublet,vEI,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sEPSP,sIPSP0,vRange,pp0,idt,avoidSpike);
                if dtplot
            ...
            if testIE
                ...
                [kVIE(:,idt,:),pv,vadd,vIEDoublet,vIE,h] = doubleCheck(silico,name,para,vv0,bool,tstep,iidt,dur,i,vleakage,sIPSP,sEPSP0,vRange,pp0,idt,avoidSpike);
                if dtplot
        ...
        if draw
            ...
            if meshK
                ...
                if testIE
                    ...
                    hold on
                    for iv0 = 1:nv0
                        plot3(X,Y,kVIE(:,:,iv0),'Color',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('I(t1)-E(t2) (ms)');
                    zlabel('k');
                    ...
                    printpic(hIE,diri,fname,meshformat,printDriver,dpi,pos0);
                    campos([0.1*max(dtRange),0.2*max(t),0.8*klim(2)]);
                    disp('IE');
                end
                if testEI
                    hEI = figure;
                    hold on
                    for iv0 = 1:nv0
                        plot3(X,Y,kVEI(:,:,iv0),'Color',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('E(t1)-I(t2) (ms)');
                    zlabel('k');
                    ...
                    printpic(hEI,diri,fname,meshformat,printDriver,dpi,pos0);
                    campos([0.1*max(dtRange),0.2*max(t),0.8*klim(2)]);
                    disp('EI');
                end
                if testEE
                    ...
                    hold on
                    for iv0 = 1:nv0
                        plot3(X,Y,kVEE(:,:,iv0),'Color',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('E(t1)-E(t2) (ms)');
                    zlabel('k');
                    ...
                    printpic(hEE,diri,fname,meshformat,printDriver,dpi,pos0);
                    disp('EE');
                end
                if testII
                    hII = figure;
                    hold on
                    for iv0 = 1:nv0
                        plot3(X,Y,kVII(:,:,iv0),'Color',c(iv0,:));
                    end
                    xlabel('time (ms)');
                    ylabel('I(t1)-I(t2) (ms)');
                    zlabel('k');
                    ...
                    disp('meshed');
                    printpic(hII,diri,fname,meshformat,printDriver,dpi,pos0);
                    disp('II');
                end
                toc;
            else
                ...
                if testEE
                    for iv0 = 1:nv0
                        for idt = 1:ndt
                            tmp = abs(kVEE((idtRange(idt)+2):nt,idt,iv0) - kVEE((idtRange(idt)+1):nt-1,idt,iv0));
                            itmp = find(tmp>kThres,1,'first');
                            if ~isempty(itmp)
                                disp(['check kVEE ', num2str(iv0),'th v, ',num2str(idt),'th dt, ', num2str(itmp+idtRange(idt)+1),'th time step']);
                            end
                        end
                    end
                end
                if testEI
                    for iv0 = 1:nv0
                        for idt = 1:ndt
                            tmp = abs(kVEI((idtRange(idt)+2):nt,idt,iv0) - kVEI((idtRange(idt)+1):nt-1,idt,iv0));
                            itmp = find(tmp>kThres,1,'first');
                            if ~isempty(itmp)
                                disp(['check kVEI ', num2str(iv0),'th v, ',num2str(idt),'th dt, ', num2str(itmp+idtRange(idt)+1),'th time step']);
                            end
                        end
                    end
                end
                if testIE
                    for iv0 = 1:nv0
                        for idt = 1:ndt
                            tmp = abs(kVIE((idtRange(idt)+2):nt,idt,iv0) - kVIE((idtRange(idt)+1):nt-1,idt,iv0));
                            itmp = find(tmp>kThres,1,'first');
                            if ~isempty(itmp)
                                disp(['check kVIE ', num2str(iv0),'th v, ',num2str(idt),'th dt, ', num2str(itmp+idtRange(idt)+1),'th time step']);
                            end
                        end
                    end
                end
                if testII
                    for iv0 = 1:nv0
                        for idt = 1:ndt
                            tmp = abs(kVII((idtRange(idt)+2):nt,idt,iv0) - kVII((idtRange(idt)+1):nt-1,idt,iv0));
                            itmp = find(tmp>kThres,1,'first');
                            if ~isempty(itmp)
                                disp(['check kVII ', num2str(iv0),'th v, ',num2str(idt),'th dt, ', num2str(itmp+idtRange(idt)+1),'th time step']);
                            end
                        end
                    end
                end
...
function [EPSP,IPSP,E_tmax,I_tmax,vleakage] = sPSP_check(silico,tstep,vRange,fIRange,fERange,para,bool,name,dur,i,v0id,dtRange,avoidSpike)
    ndt = length(dtRange);
    nv0 = length(vRange);
    nE = length(fERange);
    nI = length(fIRange);
...
function [kV,pV,vaddV,vDoubletV,EI,h0] = doubleCheck(silico,name,para,v0,bool,tstep,iidt,dur,i,vleakage,sPSP1,sPSP2,vRange,pp,idt,avoidSpike)
    %avoidSpike = false;
    global nvplot iv0Case
    n1 = size(sPSP1,2);
    n2 = size(sPSP2,2);
    ...
    m = zeros(nt,n1*n2,nv0);
    n = zeros(nt,n1*n2,nv0);
    h = zeros(nt,n1*n2,nv0);
